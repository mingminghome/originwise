...
| **Gemini Search (web)** | Split by family in AI Studio | **Gemini 3 Search** is often **0/0** (text Flash still works). **Gemini 2.5 / Gemini 2 Search** are typically **1.5K RPD**. **Default Search (~1.5K RPD)** covers robotics ER. OriginWise tries **robotics ER, then 2.5/2.0, then Gemini 3 last**. Pin: `GEMINI_WEB_MODEL=gemini-robotics-er-2-preview`. Or enable billing / `WEB_LOOKUP=off`. |
