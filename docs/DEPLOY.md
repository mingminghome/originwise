...
| **Gemini Search (web)** | Split by family in AI Studio | **Gemini 3 Search** is often **0/0** (Flash agents still work without Search). **Gemini 2.5 Search** may show RPD while `gemini-2.5-flash` is blocked for new users. **Default Search (~1.5K RPD)** covers models such as `gemini-robotics-er-2-preview`, robotics ER 1.6, deep-research agents, Gemma 4 — OriginWise web research tries **robotics ER first** so free Default RPD is usable. Pin: `GEMINI_WEB_MODEL=gemini-robotics-er-2-preview`. Or enable billing / `WEB_LOOKUP=off`. |
